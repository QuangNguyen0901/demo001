...
class FormHelperTest extends TestCase
{
    ...
    public function testFormSecureWithCustomNameAttribute()
    {
        ...
        $this->Form->text('UserForm.published', ['name' => 'Users[custom]']);
        $this->assertEquals('Users.custom', $this->Form->fields[0]);

        $this->Form->text('UserForm.published', ['name' => 'Users[custom][another][value]']);
    ...
    public function testControlOverridingMagicSelectType()
    {
        ...
        $expected = [
            'div' => ['class' => 'input text'],
            'label' => ['for' => 'model-user-id'], 'Users', '/label',
    ...
    {
        $result = $this->Form->control('Users.get_spam', [
            'type' => 'checkbox',
            'value' => '0',
            'hiddenField' => '1',
        ]);
        $expected = [
            ...
            ['input' => [
                'type' => 'hidden', 'name' => 'Users[get_spam]',
                'value' => '1'
            ]],
            ['input' => [
                'type' => 'checkbox', 'name' => 'Users[get_spam]',
    ...
    public function testHtml5Controls()
    {
        ...
        $expected = [
            'input' => ['type' => 'email', 'name' => 'Users[email]']
        ];
        $this->assertHtml($expected, $result);

        $result = $this->Form->search('Users.query');
        $expected = [
            'input' => ['type' => 'search', 'name' => 'Users[query]']
        ];
        $this->assertHtml($expected, $result);

        $result = $this->Form->search('Users.query', ['value' => 'test']);
        $expected = [
            'input' => ['type' => 'search', 'name' => 'Users[query]', 'value' => 'test']
        ];
        $this->assertHtml($expected, $result);

        $result = $this->Form->search('Users.query', ['type' => 'text', 'value' => 'test']);
        $expected = [
            'input' => ['type' => 'text', 'name' => 'Users[query]', 'value' => 'test']
        ];
        $this->assertHtml($expected, $result);
    }
